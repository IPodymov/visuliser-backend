...
2.  **Programs (Образовательные программы)**:
    ...
    - **Нормализованная база данных**: 
        - Использование справочников (`Faculty`, `Direction`) для целостности данных.
        - Разделение сущностей `Discipline` (уникальный каталог названий) и `ProgramDiscipline` (реализация дисциплины в программе) для оптимизации хранения и ускорения поиска.
    - **Оптимизация производительности**:
        - Использование `select_related` и `prefetch_related` для минимизации SQL-запросов (N+1).
        - Индексация ключевых полей (`db_index=True`).
        - Кэширование API-ответов (15 минут) с автоматическим сбросом при обновлении данных (Signals).
    - **Импорт данных**:
        - Парсинг Excel-файлов (.xlsx) с учебными планами.
        - Умное обновление существующих записей.
