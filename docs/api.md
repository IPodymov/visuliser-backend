...
- **URL**: `/programs/chart-data/`
- **Method**: `GET`
- **Параметры запроса**:
  - `ids`: Список ID программ (обязательно).
  - `type`: Тип графика. Опционально. Значения:
    - `competency` (по умолчанию) — Сравнение по компетенциям.
    - `credits` — Сравнение по зачетным единицам (ЗЕТ) в блоках.
    - `disciplines` — Сравнение по общему количеству дисциплин.
- **Response**: Структура данных для построения диаграмм (JSON).
